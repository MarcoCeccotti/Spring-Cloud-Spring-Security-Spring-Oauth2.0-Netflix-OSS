...
public class LoginController {
	...
	public ResponseEntity<?> checkForgottenPassword(@RequestBody AccessParameters access_params,
		...
		return ResponseEntity
					.status(HttpStatus.OK)
					.body(new WrapperResponse("Recupero password avvenuto con successo. Controlla la tua posta elettronica.", null));
	}
	
	@ApiOperation(value = "Registrates a new user")
	@PostMapping(value = "/registrate", consumes = MediaType.APPLICATION_JSON, produces = MediaType.APPLICATION_JSON)
	public ResponseEntity<?> registrateUser(@RequestBody Users user) {
		
		if (!userService.checkUserToRegistrate(user)) {
			return ResponseEntity
						.status(HttpStatus.BAD_REQUEST)
						.body(new WrapperResponse(null, new Outcome(HttpStatus.BAD_REQUEST, "Utente già registrato, prego digitare altre credenziali", Type.ERROR, null, null))); 
		}
		
		return ResponseEntity
					.status(HttpStatus.OK)
					.body(new WrapperResponse(user, null));
	}
}